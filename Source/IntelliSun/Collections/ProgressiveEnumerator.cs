...
namespace IntelliSun.Collections
{
    public class ProgressiveEnumerator<T> : IEnumerable<T>, IEnumerator<T>
    {
        ...
        public ProgressiveEnumerator(IEnumerable<T> enumerable)
        {
            if (enumerable == null)
                throw new ArgumentNullException("enumerable");

            this.items = Prepare(enumerable);
            this.preloaded = this.items.Count != 0;

            this.enumerator = this.preloaded
                ? this.items.GetEnumerator()
                : enumerable.GetEnumerator();
        }

        private static IList<T> Prepare(IEnumerable<T> enumerable)
        {
